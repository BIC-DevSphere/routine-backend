...
export class RoutineSyncService {
	...
	async syncDailyRoutine(token: string, date = "2025-11-13") {
		try {
			...
			console.log(`DB: ${dbRoutines.length}, API: ${apiResponse.list.length}`);
			...
			if (newHashes.size > 0) {
				...
				for (const hash of newHashes) {
					...
					try {
						const result = await routineService.addRoutineEntryToDb(apiRoutine);
						if (result?.success) {
							results.added++;
							console.log(
						...
						results.errors.push(`Error adding: ${apiRoutine.moduleDto?.name}`);
...
export const routineSyncService = new RoutineSyncService();
